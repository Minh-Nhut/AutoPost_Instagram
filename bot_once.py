...
def _write_pw_worker():
    ...
    lines = [
        ...
        '                time.sleep(0.5)\n',
        '\n',
        '                # Phương pháp: dispatch input events để React nhận\n',
        '                # Instagram dùng React nên cần trigger nativeInputValueSetter hoặc input event\n',
        '                ok = page.evaluate(f"""\n',
        '                    (captionText) => {{\n',
        ...
        '                    }}\n',
        '                """)\n',
        '                log(f"   📌 Caption value sau insert: [{typed_val[:50] if typed_val else \'\'}...]")\n',
        '\n',
        '                if typed_val and len(typed_val.strip()) > 3:\n',
        '                    log(f"   ✅ Caption OK ({len(typed_val)} ký tự) via {ok} + {sel}")\n',
        '                    caption_typed = True\n',
        '                    break\n',
        '                else:\n',
        '                    log(f"   ⚠️ Caption rỗng sau {ok}, thử xclip...")\n',
        '                    # Phương pháp 2: xclip clipboard paste\n',
        '                    try:\n',
        '                        import subprocess as sp\n',
        '                        # Thử xclip\n',
        '                        try:\n',
        '                            sp.run(["xclip", "-selection", "clipboard"], input=caption.encode("utf-8"), check=True, timeout=5)\n',
        '                            clip_tool = "xclip"\n',
        '                        except Exception:\n',
        '                            sp.run(["xdotool", "type", "--clearmodifiers", "--delay", "0", "--", caption], check=True, timeout=30)\n',
        '                            clip_tool = "xdotool"\n',
        '                        if clip_tool == "xclip":\n',
        '                            el.click()\n',
        '                            time.sleep(0.3)\n',
        ...
        '                            time.sleep(0.8)\n',
        '                        typed_val2 = page.evaluate(f"""\n',
        '                            () => {{\n',
        '                                const el = document.querySelector(\'{sel}\');\n',
        '                                return el ? (el.innerText || el.value || "").trim() : "";\n',
        '                            }}\n',
        '                        """)\n',
        '                        log(f"   📌 Caption sau {clip_tool}: [{typed_val2[:50] if typed_val2 else \'\'}...]")\n',
        '                        if typed_val2 and len(typed_val2.strip()) > 3:\n',
        '                            log(f"   ✅ Caption OK ({len(typed_val2)} ký tự) via {clip_tool}")\n',
        '                            caption_typed = True\n',
        '                            break\n',
        '                    except Exception as clip_e:\n',
        '                        log(f"   ⚠️ Clipboard/xdotool lỗi: {clip_e}")\n',
        '            except Exception as e:\n',
        '                log(f"   ⚠️ Caption sel [{sel}]: {e}")\n',
        '\n',
        '        if not caption_typed:\n',
        '            # Fallback cuối: tìm trong tất cả dialog, dùng execCommand\n',
        '            log("   🔄 Fallback: tìm contenteditable trong dialog...")\n',
        '            try:\n',
        ...
        '                            const els = Array.from(scope.querySelectorAll(\'[contenteditable="true"], div[role="textbox"], textarea\'));\n',
        '                            const el = els.find(e => e.offsetParent !== null);\n',
        '                            if (!el) continue;\n',
        '                            el.focus();\n',
        '                            el.textContent = "";\n',
        '                            const ok = document.execCommand("insertText", false, captionText);\n',
        '                            if (!ok) {{\n',
        '                                el.textContent = captionText;\n',
        '                                el.dispatchEvent(new Event("input", {{ bubbles: true }}));\n',
        '                            }}\n',
        '                            const val = (el.innerText || el.value || el.textContent || "").trim();\n',
        '                            return val.length;\n',
        '                        }}\n',
        '                        return 0;\n',
        '                    }}\n',
        '                """, caption)\n',
        '                log(f"   📌 Fallback caption length: {result}")\n',
        '                if result and result > 3:\n',
        '                    log(f"   ✅ Caption qua JS dialog fallback ({result} ký tự)")\n',
        '                    caption_typed = True\n',
        '            except Exception as e:\n',
