...
def _write_pw_worker():
    ...
    lines = [
        ...
        '            sys.exit(4)\n',
        '\n',
        '        # ── Chờ xác nhận & lấy URL bài đăng ──\n',
        '        log("   ⏳ Chờ xác nhận bài đã đăng...")\n',
        '\n',
        '        # Instagram hiện popup "Bài viết đã được chia sẻ" / "Your post has been shared"\n',
        '        # Trong popup có link "Xem bài viết" / "View post" dẫn đến /p/<shortcode>/\n',
        '        post_url = ""\n',
        '        for wait_i in range(20):  # chờ tối đa 30s\n',
        '            time.sleep(1.5)\n',
        '            try:\n',
        '                # Kiểm tra lỗi UI trước\n',
        '                for err_sel in [\n',
        '                    \'[role="alert"]:has-text("went wrong")\',\n',
        '                    \'[role="alert"]:has-text("xảy ra lỗi")\',\n',
        ...
        '                            log(f"   ❌ Phát hiện lỗi UI: {err_sel}")\n',
        '                            page.screenshot(path="/tmp/debug_after_post.png")\n',
        '                            log("ERR:POST_FAILED_UI_ERROR")\n',
        '                            sys.exit(7)\n',
        '                    except: pass\n',
        '\n',
        '                # Tìm link bài đăng trong popup thành công\n',
        '                found_url = page.evaluate("""\n',
        '                    () => {\n',
        '                        // Tìm link /p/<shortcode>/ trong toàn bộ trang\n',
        '                        const links = Array.from(document.querySelectorAll("a[href]"));\n',
        '                        const postLink = links.find(a => /\\/p\\/[A-Za-z0-9_-]{8,}\\/?/.test(a.getAttribute("href")));\n',
        '                        if (postLink) return postLink.getAttribute("href");\n',
        '                        // Fallback: tìm trong text body "instagram.com/p/"\n',
        '                        const bodyText = document.body.innerHTML;\n',
        '                        const m = bodyText.match(/href="(\\/p\\/[A-Za-z0-9_-]+\\/?)"/);\n',
        '                        if (m) return m[1];\n',
        '                        return null;\n',
        '                    }\n',
        '                """)\n',
        ...
        '                        post_url = "https://www.instagram.com" + found_url\n',
        '                    else:\n',
        '                        post_url = found_url\n',
        '                    log(f"   ✅ Tìm thấy URL bài đăng: {post_url}")\n',
        '                    break\n',
        '\n',
        '                # Kiểm tra popup thành công xuất hiện chưa (dù chưa có link)\n',
        '                success_shown = page.evaluate("""\n',
        '                    () => {\n',
        '                        const txt = (document.body.innerText || "").toLowerCase();\n',
        '                        return txt.includes("bài viết đã được chia sẻ") ||\n',
        '                               txt.includes("your post has been shared") ||\n',
        '                               txt.includes("post has been shared") ||\n',
        '                               txt.includes("đã được chia sẻ");\n',
        '                    }\n',
        '                """)\n',
        '                if success_shown:\n',
        '                    log(f"   ✅ Popup thành công xuất hiện (lần {wait_i+1}), đợi link...")\n',
        '                else:\n',
        '                    log(f"   ⏳ Chờ popup xác nhận... ({wait_i+1}/20)")\n',
        '            except Exception as we:\n',
        '                log(f"   ⚠️ Lỗi chờ xác nhận: {we}")\n',
        '\n',
        '        page.screenshot(path="/tmp/debug_after_post.png")\n',
        '        log(f"   📸 Screenshot sau đăng: /tmp/debug_after_post.png")\n',
        '        log(f"   🔗 URL trang hiện tại: {page.url}")\n',
        '\n',
        '        context.storage_state(path=SESSION_FILE)\n',
        '        if post_url:\n',
        '            post_id = post_url\n',
        '            log(f"   ✅ Đăng thành công, URL: {post_id}")\n',
        '        elif "/p/" in page.url:\n',
        '            post_id = page.url\n',
        '            log(f"   ✅ Đăng thành công (từ page.url): {post_id}")\n',
        '        else:\n',
        '            # Không lấy được URL nhưng không có lỗi → vẫn coi là thành công\n',
        '            post_id = "ig_" + str(int(time.time()))\n',
        '            log(f"   ⚠️ Không lấy được URL bài đăng, dùng timestamp ID: {post_id}")\n',
        '        log(f"OK:{post_id}")\n',
        '        browser.close()\n',
        '\n',
        '    except Exception as e:\n',
